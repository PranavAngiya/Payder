...
router.post('/', async (req, res) => {
    ...
    try{
        ...
        if(!username || !updated_risk_tolerance || !updated_experience || !updated_goals){
            ...
            //Create sentiment row for a user in the sentiment table
            query = 'INSERT INTO sentiment (tolerance, experience, goals, user_id) VALUES (?, ?, ?, ?)';
            await db.query(query, [tolerance, experience, goals, userID]);
            return res.status(200).json({message: "Sentiment updated."});
